...
function setPlainTextTo(el, plain){
  ...
  // ★ 不要改 display，保留 .page 的 display:flex
  // el.style.display    = 'block'; // ← 這行刪掉，避免版面被打壞

  // 讓它能選取與有插入點（覆蓋全域 user-select:none）
...
/* ========== 溢出檢測 + 容量估算（臨界多一行修正＋高度異常保險） ========== */
function isOverflow(el){
  if (!el) return false;

  // 若容器高度異常，視為不溢出，避免一輸入就被「全推走」
  ...
  return free < lh * 0.35; // 剩餘 < 0.35 行 視為不安全（臨界也當溢出）
}
...
function flowOverflowFrom(dbIndex){
  ...
  // 防護：避免極端情境無限循環
  let guard = 0, guardMax = 500; // 超長貼上最多推 500 步
