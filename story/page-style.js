...
(function(){
  ...
  function switchTo(style){
    ...
    if (style === 'novel'){
      page.type = 'novel'; page.image_url = '';
      page.content_json = { text_plain: plainNow || '', text_html: toHTMLFromPlain(plainNow) };
    } else if (style === 'divider-light' || style === 'divider-dark'){
      page.type = (style === 'divider-light') ? 'divider_white' : 'divider_black'; page.image_url = '';
      page.content_json = { text_plain: plainNow || '', text_html: toHTMLFromPlain(plainNow) };
    } else if (style === 'illustration'){
      if ((plainNow||'').trim().length > 0){ alert('此頁仍有文本，請先清空文本再切換成圖片頁。'); return; }
      const u = prompt('輸入圖片網址（留空=取消）', page.image_url || '');
      if (!u || !u.trim()){ page.type = 'novel'; page.image_url = ''; page.content_json = { text_plain:'', text_html:'' }; }
      else { page.type = 'image'; page.image_url = u.trim(); page.content_json = { text_plain:'', text_html:'' }; }
    }
    lightRedraw();
  ...
  function bindImageEditors(){
    ...
    for (let i=0;i<list.length;i++){
      ...
      if (pageEl.__imgEditBound) continue; pageEl.__imgEditBound = true;
      pageEl.addEventListener('dblclick', ()=>{
        const u = prompt('輸入圖片網址（留空=改回一般頁）', p.image_url || '');
        if (!u || !u.trim()){ p.type = 'novel'; p.image_url = ''; p.content_json = { text_plain:'', text_html:'' }; }
        else { p.image_url = u.trim(); }
        lightRedraw();
        try{ persistDraft && persistDraft(); }catch(_){}
      });
    }
